...
<a class="titleList">LISTA DE CHIPS</a><br><br>
...
<table width="98%">
    ...
    <tbody> 
        <?php
        $sql = "SELECT * FROM vendas_chips;";
        $query = mysqli_query($conexao,$sql);
        while ($row = mysqli_fetch_assoc($query)){ // comando que era da linha 43 = <td>'.$row['id'].'</td>
            echo '
                <tr class="downLINE">
                    <td class="formatList">'.$row['qtd_chips_25'].'</td>
                    <td class="formatList">'.$row['qtd_chips_30'].'</td>
                    <td class="formatList">'.$row['qtd_total_chips'].'</td>
                    <td class="formatList">R$'.$row['inv_chips'].'</td>
                    <td class="formatList">R$'.$row['lucro_bruto_chips'].'</td>
                    <td class="formatList">R$'.$row['lucro_liq_chips'].'</td>
...
<!--BOTÕES-->
<div class="tamanho">
<div><form action="?pagina=apaga-chips" method="POST">
